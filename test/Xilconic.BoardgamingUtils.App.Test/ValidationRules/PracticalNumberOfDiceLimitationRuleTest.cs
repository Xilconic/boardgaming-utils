...
namespace Xilconic.BoardgamingUtils.App.Test.ValidationRules
{
    ...
    public class PracticalNumberOfDiceLimitationRuleTest
    {
        ...
        {
            // Call
            var validationRule = new PracticalNumberOfDiceLimitationRule();
            ...
            Assert.AreEqual("Number of dice", validationRule.ParameterName);

            Assert.AreEqual(1, validationRule.Minimum);
            Assert.AreEqual(50, validationRule.Maximum);
        }
        ...
        public void Validate_ValueNullOrWhitespace_ReturnInvalid(string invalidValue)
        {
            ...
            Assert.AreEqual("Number of dice must be specified.", result.ErrorContent);
        }
        ...
        public void Validate_InvalidNumberOfDieSides_ReturnInvalid(int invalidNumberOfSides)
        {
            ...
            // Assert
            Assert.IsFalse(result.IsValid);
            Assert.AreEqual("Number of dice must be in range of [1, 50].", result.ErrorContent);
        }
        ...
        public void Validate_NumberOfDieSidesNotIntegerString_ReturnInvalid()
        {
            ...
            // Assert
            Assert.IsFalse(result.IsValid);
            Assert.AreEqual("Number of dice must be a whole number.", result.ErrorContent);
        }
        ...
        public void Validate_NumberOfDieSidesTooLarge_ReturnInvalid(int extremeValue)
        {
            ...
            // Assert
            Assert.IsFalse(result.IsValid);
            Assert.AreEqual("Number of dice is too large or too small.", result.ErrorContent);
        }
